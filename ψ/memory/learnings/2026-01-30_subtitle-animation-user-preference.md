...
```css
/* Good: Minimal CSS, no distracting effects */
.word {
  opacity: 0;
...
- User also prefers emoji icons over SVG (authenticity over polish)
- Previous session established "no unnecessary animation" preference

## Tags

`animation`, `ux`, `subtitles`, `accessibility`, `user-feedback`, `iteration`
